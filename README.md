...
このエミュレーターはすべての状態をメモリ上で管理しているため、**アプリケーションを停止・再起動するたびに以下の値がリセット**されます。

| リセットされる値 | 対象デバイス | 備考 |
|---|---|---|
| SOC (State of Charge) | 蓄電池、V2H | 起動時に初期値（Settings設定値ベース）に戻ります |
| 積算充電電力量 (0xA8 / 0xD6) | 蓄電池、V2H | 0 にリセットされます |
| 積算放電電力量 (0xA9 / 0xD8) | 蓄電池、V2H | 0 にリセットされます |
| 積算発電量 (0xE1) | 太陽光発電 | 0 にリセットされます |
| 積算電力量 (0xE0, 0xE3) | スマートメーター | 0 にリセットされます |
| 積算消費電力量 (0x85) | 家庭用エアコン | 0 にリセットされます |

### Settings変更後の再起動が必要な項目

以下の設定は変更後に**アプリケーションの再起動が必要**です。変更しただけでは反映されません。

- **Wi-SUN設定** (Bルート認証ID / パスワード / デバイスパス)
- **Wi-Fi有効/無効設定** (各デバイスのON/OFF)

Settingsタブの他の項目（容量値・識別番号・メーカーコード等）は再起動なしで保存されますが、起動中のECHONET Liteプロパティ値への即時反映は保証されません。

### シナリオ実行中のスライダー操作

DashboardタブでScenario Activeがオンの場合、スライダーで手動設定した値はシナリオによって上書きされます。手動操作でプロパティ値を固定したい場合は、**Scenario ActiveをOFFにしてください**。

### ECHONETマルチキャストとWindowsの制限

Windows環境（特にDocker Desktop）では、ECHONET LiteのUDPマルチキャスト（224.0.23.0:3610）が正しく動作しない場合があります。HEMSコントローラー等から機器が検索できない場合は、**Python直接実行**に切り替えてください。
