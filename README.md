# Home Energy Emulator with ECHONET Lite interface

<img width="840" height="976" alt="スクリーンショット 2026-04-04 110656" src="https://github.com/user-attachments/assets/b974e83a-7a2f-48ae-b08a-911c76bca243" />

ECHONET Liteプロパティに対応したHEMS (Home Energy Management System) 対応機器エミュレーターです。
スマートメーター (Bルート対応)、太陽光発電システム、蓄電池システム、電気自動車充放電器 (V2H)、電気給湯器、瞬間式給湯器、家庭用エアコン、エネファーム（燃料電池コージェネレーションシステム）、分電盤メータリング をエミュレートし、NiceGUIによるWebインターフェースで状態監視・操作が可能です。
...
- **Web UI**:
    - **Dashboard**: 各デバイスの発電・充放電状態や消費電力のリアルタイム監視と、スライダーによる手動シミュレーション操作。天候API（Open-Meteo）を使った太陽光発電の自動減衰・予測グラフ表示にも対応。
...
- **ECHONET Liteプロパティ対応**:
    ...
    - **瞬間式給湯器**: 動作状態 (0x80)、給湯器燃焼状態 (0xD0)、給湯温度設定値 (0xD1)、風呂湯量設定4 (0xD4, 0xD5)、風呂温度設定値 (0xE1)、風呂給湯器燃焼状態 (0xE2)、風呂自動モード設定 (0xE3)、風呂追い焚き動作設定 (0xE4)、沸き上げモード (0xEF) など。
    - **家庭用エアコン**: 運転状態 (0x80)、運転モード設定 (0xB0)、温度設定値 (0xB3)、風量設定 (0xA0)、節電動作設定 (0x8F)、瞬時消費電力計測値 (0x84)、積算消費電力量計測値 (0x85)。
    - **エネファーム（燃料電池: 0x027C）**: 動作状態 (0x80)、系統連系状態 (0xD0)、瞬時発電電力 (0xC4)、積算発電電力量 (0xC5)、定格発電出力 (0xC2)、発電動作状態 (0xCB) など。
    - **分電盤メータリング (0x0287)**: メインブレーカーおよび各分岐回路（計測チャンネル）の瞬時・積算電力値を以下の通り動的に紐づけて応答します。
        - **主幹 (0xC0, 0xC1, 0xC6, 0xC7)**: スマートメーターの買電量・売電量・瞬時電力・電流と連動。
        - **CH1 (0xB3, 0xB7, 0xD0)**: 電気給湯器（エコキュート）の消費電力・積算と連動。
        - **CH2 (0xB3, 0xB7, 0xD1)**: 家庭用エアコンの消費電力・積算と連動。
        - **CH3 (0xB3, 0xB7, 0xD2)**: シナリオの `load_w`（その他負荷）の瞬時電力・積算と連動。
## 必要要件
- **ハードウェア環境**:
...
### Dashboard タブ
- **System Status**: 現在のグリッド電力（売買電）、太陽光発電量、蓄電池・V2H・電気給湯器・瞬間式給湯器・エアコンの状態をカード形式で表示します。
...
### Settings タブ
- **Simulation & Weather Settings**: UI上のインタラクティブなマップ（Leaflet）をクリックして、天候情報取得の基準となる地理座標（緯度・経度）を直感的に設定できます。
- **Wi-SUN Settings**: Bルート認証ID・パスワードを設定します。
- **Network Devices**: Wi-Fi経由で公開するECHONET Liteデバイス（太陽光・蓄電池・電気給湯器・瞬間式給湯器・V2H・エアコン・エネファーム・分電盤）を個別に有効/無効化できます。
...
> **⚠️ エネファーム使用時の注意**: Fuel Cell (0x027C) を有効にする場合は、**Inst. Water Heater (0x0272) も同時に有効にし、他のすべてのWi-Fiデバイスは無効にしてください**。エネファームの発電動作状態 (`0xCB`) は瞬間式給湯器の燃焼状態 (`0xD0`) に連動します。
...
| リセットされる値 | 対象デバイス | 備考 |
|---|---|---|
| SOC (State of Charge)、残湯量 | 蓄電池、V2H、電気給湯器 | 起動時に50%に初期化されます |
| 積算電力量 (充電・放電・発電など) | 蓄電池、V2H、太陽光発電、スマートメーター、エアコン、電気給湯器、エネファーム、分電盤 | 0 にリセットされます |
