...
スマートメーター (Bルート対応)、太陽光発電システム、蓄電池システム、電気自動車充放電器 (V2H)、電気給湯器をエミュレートし、NiceGUIによるWebインターフェースで状態監視・操作が可能です。
...
- **Debug Controls**:
    ...
    - **Manual Sliders**: Load（負荷）、Solar（発電）、Battery/V2H（充放電）、Water Heater（給湯量・加熱）の値を手動で操作し、ECHONET Liteプロパティにリアルタイムで反映させることができます。
